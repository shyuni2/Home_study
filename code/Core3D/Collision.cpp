...
bool Collision::IntersecRect(Rect rt1, Rect rt2, Rect* pRect)
{
	Rect rt;
...
{	
	Rect rtInterction;
	int ret = IntersecRect(rt1, rt2, &rtInterction);
	if (ret <= 0)
	{
		return RECT_OUT;
	}
...
bool   Collision::IntersecBox(Box rt1, Box rt2, Box* pRect)
{
	Box rt;
...
{
	Box rtInterction;
	int ret = IntersecBox(rt1, rt2, &rtInterction);
	if (ret <= 0)
	{
		return RECT_OUT;
	}
